...
function App() {
    ...
    const {
        mouthState, baseMouthState, setMouthStateBase, toggleTalking,
        isTalkingContinuously,
    } = useAnimatedMouth({ initialMouthState: "neutral" });
    const {
        isRecording, statusMessage: sttStatusMessage, // Rename to avoid conflict
    ...
    const [allAnimals, setAllAnimals] = useState<string[]>([]);
    ...
    const [userPossibleAnimals, setUserPossibleAnimals] = useState<string[]>([]); // User's list of AI possibilities (for UI hints)
    ...
    const [lastUserAnswer, setLastUserAnswer] = useState<"yes" | "no" | null>(null);
    const [messageToSpeak, setMessageToSpeak] = useState<string>(""); // Text for TTS
    const [displayMessage, setDisplayMessage] = useState<string>("Loading game..."); // Message shown below face
    ...
    useEffect(() => { isTalkingContinuouslyRef.current = isTalkingContinuously; }, [isTalkingContinuously]);
    ...
    useEffect(() => {
        const stopMouthIfNeeded = () => {
            if (isTalkingContinuouslyRef.current) {
                console.log("App TTS: Stopping mouth animation (speech ended/cancelled/error).");
        ...
        if (messageToSpeak && messageToSpeak.trim() !== "") {
            ...
            if ("speechSynthesis" in window) {
                ...
                utterance.onstart = () => {
                    if (!isTalkingContinuouslyRef.current) {
                        console.log("App TTS: Speech started - Activating mouth animation.");
                        toggleTalking();
                    }
                };
                utterance.onend = () => {
        ...
        return () => {
            if ("speechSynthesis" in window && window.speechSynthesis.speaking) {
                console.log("App TTS Cleanup: Cancelling speech on effect re-run/unmount.");
        ...
        if (gamePhase === GamePhase.LOADING) {
            ...
            const initializeGame = async () => {
                try {
                    ...
                    if (animalsResponse.error || !animalsResponse.animals || animalsResponse.animals.length === 0) {
                        throw new Error(animalsResponse.error || "Failed to load animal list or list is empty.");
                    }
                    ...
                    setUserPossibleAnimals([...animals]); // Initialize User's possibilities
            ...
            const selectAnimal = async () => {
                try {
                    const response = await apiSelectAnimal();
                     ...
                     // Start the game
                     const startMessage = "I have chosen my secret animal. Your turn to ask a question. Hold Space to speak.";
                    ...
                    setErrorMessage(`AI animal selection failed: ${error instanceof Error ? error.message : String(error)}`);
                    setGamePhase(GamePhase.ERROR);
                }
            };
            selectAnimal();
    ...
    useEffect(() => {
        if (gamePhase === GamePhase.AI_PROCESSING_QUESTION && finalTranscriptionRef.current) {
            ...
            if (!userQuestion || userQuestion.trim() === "" || userQuestion.includes("[Silence")) {
                console.log("User question was empty or silent.");
                 const retryMsg = "I didn't hear a question. Please ask again. Hold Space to speak.";
            ...
            const askAi = async () => {
                try {
                    const response = await apiAsk({ question: userQuestion, secret_animal: aiSecretAnimal });
                     if (response.error || !response.answer) {
                         throw new Error(response.error || "AI failed to answer the question.");
                     }
                    ...
                    setErrorMessage(`Error getting AI answer: ${error instanceof Error ? error.message : String(error)}`);
     ...
     useEffect(() => {
        if (gamePhase === GamePhase.AI_ANSWERING && lastAiAnswer) {
            ...
            // TODO: Optional - Implement User-side filtering based on `lastUserQuestion` and `answer`
            // This would update `userPossibleAnimals` for UI hints.
            // Example: const updatedList = filterUserList(userPossibleAnimals, lastUserQuestion, answer);
            // setUserPossibleAnimals(updatedList);
             // if (updatedList.length === 1) { /* USER CAN GUESS? */ }
             // if (updatedList.length === 0) { /* User made an error? */ }
     ...
     }, [gamePhase, lastAiAnswer, userPossibleAnimals, lastUserQuestion]); // Runs when AI_ANSWERING and answer is ready
             ...
             const generateQuestion = async () => {
                if (aiPossibleAnimals.length === 0) {
                     ...
                     setErrorMessage("AI Error: No possible animals remaining.");
                      setMessageToSpeak("Something went wrong, I have no possibilities left.");
                     setGamePhase(GamePhase.ERROR);
                     return;
                 }
                 ...
                 try {
                     const response = await apiGenerateQuestion({ current_list: aiPossibleAnimals });
                     if (response.error || !response.question) {
                         throw new Error(response.error || "AI failed to generate a question.");
                     }
                     ...
                     setErrorMessage(`Error generating AI question: ${error instanceof Error ? error.message : String(error)}`);
                     setGamePhase(GamePhase.ERROR);
                 }
             };
    ...
    useEffect(() => {
        if (gamePhase === GamePhase.AI_PROCESSING_ANSWER && finalTranscriptionRef.current) {
            ...
            finalTranscriptionRef.current = null; // Consume
             ...
             if (!userAnswerRaw || userAnswerRaw.trim() === "" || userAnswerRaw.includes("[Silence")) {
            ...
            // Basic Yes/No check (can be improved)
             const userAnswerClean = userAnswerRaw.toLowerCase().trim().replace(/[.!?]/g, '');
            let userAnswer: "yes" | "no" | null = null;
            if (userAnswerClean === "yes" || userAnswerClean === "yeah" || userAnswerClean === "yep") {
                userAnswer = "yes";
            } else if (userAnswerClean === "no" || userAnswerClean === "nope" || userAnswerClean === "nah") {
                userAnswer = "no";
            }
            ...
            setDisplayMessage(`You answered: "${userAnswer}" to "${lastAiQuestion}"\nAI is updating its list...`);
     ...
     useEffect(() => {
         if (gamePhase === GamePhase.AI_FILTERING && lastAiQuestion && lastUserAnswer) {
             ...
             const filterAiList = async () => {
                 try {
                     ...
                     if (kept.length === 1) {
                         ...
                         setErrorMessage("AI Error: No possibilities left based on answers.");
                          setMessageToSpeak("Hmm, based on your answers, no animal fits! Let's restart?"); // Or declare error
                     ...
                     setErrorMessage(`Error during AI filtering: ${error instanceof Error ? error.message : String(error)}`);
                     setGamePhase(GamePhase.ERROR);
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            const canRecord = gamePhase === GamePhase.USER_TURN_ASK || gamePhase === GamePhase.USER_TURN_ANSWER;

            if (event.key === TRANSCRIPTION_KEY && canRecord && !keydownTriggeredRef.current && !isRecordingRef.current) {
                ...
                finalTranscriptionRef.current = null; // Clear previous final transcription
                 ...
                 if (gamePhase === GamePhase.USER_TURN_ASK) setGamePhase(GamePhase.USER_RECORDING_QUESTION);
                 if (gamePhase === GamePhase.USER_TURN_ANSWER) setGamePhase(GamePhase.USER_RECORDING_ANSWER);
                startRecording();
            }
        };
        ...
        const handleKeyUp = (event: KeyboardEvent) => {
            if (event.key === TRANSCRIPTION_KEY) {
                 ...
                 const wasRecordingPhase = gamePhase === GamePhase.USER_RECORDING_QUESTION || gamePhase === GamePhase.USER_RECORDING_ANSWER;
                ...
                if (wasRecordingPhase && isRecordingRef.current) {
                    ...
                    console.log("App: Spacebar Up - Not in a valid recording state to stop.");
                }
            }
        };
    ...
    // Add gamePhase to dependencies to re-evaluate `canRecord`
     ...
     useEffect(() => {
         ...
         const previousPhase = gamePhase === GamePhase.USER_RECORDING_QUESTION ? GamePhase.AI_PROCESSING_QUESTION
                             : gamePhase === GamePhase.USER_RECORDING_ANSWER ? GamePhase.AI_PROCESSING_ANSWER
                             : null;
        ...
        <div className="relative min-h-screen bg-black flex flex-col">
            ...
            <div className="flex-grow flex items-center justify-center">
             ...
             <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-full max-w-4xl p-4 bg-black/80 rounded-lg shadow-lg backdrop-blur-sm">
                 ...
                 <p className="text-center text-sm text-yellow-300 font-mono h-4">
                     ...
                     {(gamePhase === GamePhase.USER_TURN_ASK || gamePhase === GamePhase.USER_TURN_ANSWER) && !isRecording ? "Hold [SPACE] to speak" : ""}
                 ...
                 {/* Game Over Controls */}
                  {(gamePhase === GamePhase.GAME_OVER_AI_WINS || gamePhase === GamePhase.GAME_OVER_USER_WINS || gamePhase === GamePhase.ERROR) && (
            ...
            <div className="absolute top-0 right-0 z-10 flex flex-wrap justify-center gap-x-8 gap-y-4 p-4 bg-zinc-800/90 rounded-lg shadow-lg max-w-3xl backdrop-blur-sm">
                 ...
                 <div className="flex flex-col gap-2 items-center w-full sm:w-auto">
                     ...
                     <div // Changed from button to div
                         className={`w-full px-4 py-2 rounded text-white font-semibold transition-colors text-sm ${
                              isRecording
                                 ...
                                 : (gamePhase === GamePhase.USER_TURN_ASK || gamePhase === GamePhase.USER_TURN_ANSWER)
                     ...
                     >
                         {isRecording ? "Recording..." :
                         (gamePhase === GamePhase.USER_TURN_ASK || gamePhase === GamePhase.USER_TURN_ANSWER) ? "Ready" : "AI Turn"}
                     </div>

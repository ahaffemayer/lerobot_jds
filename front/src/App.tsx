...
import React, { useState, useEffect, useCallback, useRef, use } from "react"; // Ajout de useRef
...
function App() {
  ...
  // --- Utiliser le hook de transcription mis à jour ---
  const {
    isRecording,
    statusMessage,
    transcriptionText,
  ...
  // Optionnel: Si une autre partie du code utilise 'isTalking', vous pouvez le recréer comme alias
  // const isTalking = isRecording;
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (
        ...
        console.log("App: Spacebar Down - Starting recording..."); // Modifié
        startRecording(); // << MODIFIÉ
      }
    };
    ...
    const handleKeyUp = (event: KeyboardEvent) => {
      if (event.key === TRANSCRIPTION_KEY) {
        ...
        if (isRecordingRef.current) {
          // << MODIFIÉ
          console.log("App: Spacebar Up - Stopping recording and sending..."); // Modifié
          stopRecordingAndSend(); // << MODIFIÉ
        } else {
          console.log("App: Spacebar Up - Recording already stopped."); // Modifié
        }
      }
    };
    ...
    return () => {
      ...
      if (isRecording) {
        // << MODIFIÉ
        console.log(
          "App: Unmounting while key might be pressed - Stopping recording..." // Modifié
        );
        stopRecordingAndSend(); // << MODIFIÉ
      }
    };
    // Dépendances mises à jour
  }, [startRecording, stopRecordingAndSend]); // << MODIFIÉ
  ...
  useEffect(() => { 
    console.log("App: isRecording state changed:", isRecording);
  }, [isRecording]); // Pour le débogage
    ...
    <div className="relative min-h-screen bg-black flex flex-col">
      ...
      <div className="absolute top-0 right-0 z-10 flex flex-wrap justify-center gap-x-8 gap-y-4 p-4 bg-zinc-800/90 rounded-lg shadow-lg max-w-3xl backdrop-blur-sm">
        ...
        <div className="flex flex-col gap-2 items-center w-full sm:w-auto">
          ...
          {/* Bouton affiche l'état (contrôlé par Espace) */}
          <button
            // ... onClick est déjà désactivé ...
            className={`w-full px-4 py-2 rounded text-white font-semibold transition-colors text-sm ${
              isRecording 
                ? "bg-red-600 hover:bg-red-500" // Style si enregistre
                : "bg-green-600 hover:bg-green-500" // Style si prêt
            } cursor-default`}
            aria-label={
              isRecording 
                ? "Enregistrement en cours (contrôlé par Espace)" // Texte modifié
                : "Prêt à enregistrer (maintenir Espace)" // Texte modifié
            }
            aria-pressed={isRecording} 
            // disabled // Reste commenté ou supprimé
          >
            {isRecording ? "Enregistrement..." : "Maintenir [ESPACE]"}{" "}
          </button>
